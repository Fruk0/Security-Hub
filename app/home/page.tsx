...
export default function SecuritySpaceRiskCalculator() {
  ...
  useUrlSync({
    ...
    validateCriterionId: (id) => !!CRITERIA.find((c) => c.id === id),
  })

  // ---------- Storage ----------
  useFrameworkStorage(state.jiraKey, state.frameworkAnswers, actions.setFrameworkAnswer)
  useNotesStorage(state.jiraKey, notes, setNotes)
  useCriteriaStorage(
    state.jiraKey,
    state.selectedCriterionId,
    state.critAnswers,
    state.critJustifications,
    actions.replaceCriteriaBuffers // atómico
  )
    ...
    <div className="mx-auto max-w-4xl p-6 space-y-6">
      <Card className="shadow-lg">
        ...
        <CardContent className="space-y-6">
          ...
          <TicketForm
            jiraKey={state.jiraKey}
            isJiraKeyValid={Boolean(state.jiraKey)}
